...
import { Clear, Close, Lock } from "@mui/icons-material";
import { US_AVOID_CONNECTIONS, uniqueAirlines } from "../../lib/airlines";
...
const FlightsList: React.FC<IFlightsListProps> = ({
  flightsData,
  numOfPass,
  upperCabin,
  error,
  ...
  const isMobileViewCalendarView = useMediaQuery(theme.breakpoints.down("md"));
  ...
  // flightsData = isSignedIn ? flightsData : flightsData.slice(0, 3);
  ...
  // const maxFlightsPoints = flightsData.reduce((maxPoints, flight) => {
  //   const maxPointsInFlight = flight.pricingDetail.reduce((max, detail) => {
  //     return detail.points > max ? detail.points : max;
  //   }, 0);
  //   return maxPointsInFlight > maxPoints ? maxPointsInFlight : maxPoints;
  // }, 0);

  // const maxFlightsPrice = flightsData.reduce((maxPrice, flight) => {
  //   const maxPriceInFlight = flight.pricingDetail.reduce((max, detail) => {
  //     return detail.cashPrice > max ? detail.cashPrice : max;
  //   }, 0);
  //   return maxPriceInFlight > maxPrice ? maxPriceInFlight : maxPrice;
  // }, 0);
  ...
  const filterAndSorting = (
    ...
    if (
      ...
      setFliteredFlights((prev: FlightData[]) => {
        ...
        if (stopValue === "1") {
          flightsDataFilteredAndSorted = flightsData.filter(
          ...
          flightsDataFilteredAndSorted = [...flightsData];
        }
        if (nousStops) {
  ...
  const clearFilters = () => {
    setFliteredFlights(flightsData);
    setStop("0");
    setSelectAirlines([]);
    setCheckedSwitch(true);
    setNoUsStops(false);
    ...
    <>
      ...
      >
        <Button
          ...
          startIcon={<SearchIcon sx={{ color: "#fff" }} fontSize="small" />}
          endIcon={<Eyes/>}
        >
          Check Calendar View
        </Button>
      ...
      <Dialog
        ...
        scroll="paper"
        fullScreen={isMobileViewCalendarView}
        sx={{
          overflow: "auto",
        }}
        maxWidth="xl"
